    ...
    class ToknSDK {
      ...
      async checkExistingVerification() {
        ...
        try {
          ...
          console.log('Raw data from checkExistingVerification:', data);
          console.log('is_16_plus:', data.age_flags.is_16_plus, 'Type:', typeof data.age_flags.is_16_plus);
          console.log('is_18_plus:', data.age_flags.is_18_plus, 'Type:', typeof data.age_flags.is_18_plus);
          console.log('is_21_plus:', data.age_flags.is_21_plus, 'Type:', typeof data.age_flags.is_21_plus);
      ...
      async exchangeCodeForToken(code) {
        ...
        console.log('ToknSDK: Callback response status:', response.status);
        console.log('ToknSDK: Callback response headers:', Object.fromEntries(response.headers.entries()));

      if (!response.ok) {
        let errorMessage = 'Token exchange failed';
        try {
          const errorData = await response.json();
          errorMessage = errorData.error_description || errorData.description || errorMessage;
        } catch (jsonError) {
      ...
      try {
        ...
        console.error('ToknSDK: Failed to parse JSON response:', jsonError);
        const responseText = await response.text();
        console.error('ToknSDK: Response text:', responseText);
      ...
      console.log('Raw data from verifyWithToken:', data);
      console.log('is_16_plus:', data.age_flags.is_16_plus, 'Type:', typeof data.age_flags.is_16_plus);
      console.log('is_18_plus:', data.age_flags.is_18_plus, 'Type:', typeof data.age_flags.is_18_plus);
      console.log('is_21_plus:', data.age_flags.is_21_plus, 'Type:', typeof data.age_flags.is_21_plus);
      ...
      async isVerified(minAge = 18) {
        ...
        console.log('ToknSDK: Raw data received in isVerified:', data);
        console.log('ToknSDK: Age flags received:', data.age_flags);
        console.log('ToknSDK: Type of is_18_plus:', typeof data.age_flags.is_18_plus, 'Value:', data.age_flags.is_18_plus);
          
          switch (minAge) {
            case 16:
            const is16Result = Boolean(data.age_flags.is_16_plus);
            console.log('ToknSDK: is_16_plus result:', is16Result);
            return is16Result;
            case 18:
            const is18Result = Boolean(data.age_flags.is_18_plus);
            console.log('ToknSDK: is_18_plus result:', is18Result);
            return is18Result;
            case 21:
            const is21Result = Boolean(data.age_flags.is_21_plus);
            console.log('ToknSDK: is_21_plus result:', is21Result);
            return is21Result;
            default:
              return false;
          }
